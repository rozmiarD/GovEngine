...
Current supported line: `govengine==0.16.10` with `sclite-core==1.0.8`.
Current published PyPI line: `govengine==0.16.10`. Older alpha releases are archived only.
...
- SCLite is published as the PyPI distribution `sclite-core`; the current supported GovEngine `0.16.x` package line depends on `sclite-core==1.0.8`.
