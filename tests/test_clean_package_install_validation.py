...
SCRIPT = ROOT / 'scripts' / 'validate_clean_package_install.py'


def test_clean_package_install_validator_dry_run_documents_clean_env_plan(tmp_path: Path) -> None:
    ...
    assert data['artifact_type'] == 'govengine_clean_package_install_validation'
    ...
    assert any('installed_surface_smoke_ok:govengine==0.15.0:surfaces=7:policy_authoring=ok' in command for command in commands)
...
def test_clean_package_install_validator_rejects_existing_venv_path(tmp_path: Path) -> None:
