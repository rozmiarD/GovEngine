...
- Source/package version: `0.16.1`.
- Published distribution: `govengine==0.16.1`.
- Release label: `0.16.1`.
- Release status: published alpha.
- Latest published PyPI package: `govengine==0.16.1`.
- Source/PyPI gap: closed for the `0.16.1` B2 enforcement-plan contract line.
...
- Admission/policy contracts: `govengine.admission` defines neutral admission, policy-decision, approval-request, and audit-record validators; `govengine.policy` adds PolicyEngine request/verdict/compiler/runtime plus digest-bound `PolicyEnforcementPlan`, existing-`GovAdmissionDecision` binding, and neutral runtime-control projection without domain policy meaning, approval workflow, audit storage, adapter, command, or live-execution ownership.
...
- PyPI publication: completed through `govengine==0.16.1`.
