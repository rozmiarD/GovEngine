...
- Source/package version: `0.14.0`.
- Release label: `0.14.0`.
- PyPI package: `govengine==0.14.0` is the published current alpha package.
...
- Admission/policy contracts: `govengine.admission` defines neutral admission, policy-decision, approval-request, and audit-record validators; `govengine.policy` adds PolicyEngine request/verdict/compiler/runtime without domain policy meaning, approval workflow, audit storage, adapter, command, or live-execution ownership.
...
- PyPI publication: completed through `govengine==0.14.0`.
