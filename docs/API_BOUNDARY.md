...
`govengine.surfaces.public_surface_index()` is the tested machine-readable map of the current alpha public surface set. It contains the neutral artifact-governance core, planning contracts, admission/policy contracts (including **PolicyEngine MVP** and B2 enforcement-plan contracts in `govengine.policy`), evidence review, domain-profile SDK, runtime contract proofs, and controlled-execution core. The published `0.16.0` line retains the prior removal of the Ravenclaw-derived optional security facade; domain behavior belongs in profiles and execution belongs in host runtimes.
