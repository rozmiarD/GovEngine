...
`govengine.surfaces.public_surface_index()` is the tested machine-readable map of the current alpha public surface set. It contains the neutral artifact-governance core, planning contracts, admission/policy contracts (including **PolicyEngine MVP** `govengine.policy`), evidence review, domain-profile SDK, runtime contract proofs, and controlled-execution core. The published `0.15.0` (`0.15.0`) line removes the prior Ravenclaw-derived optional security facade; domain security behavior belongs in the host runtime.
