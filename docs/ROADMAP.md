...
Current package baseline: `govengine==0.16.1` (`0.16.1`), depending on `sclite-core>=1.0.5,<1.1`.
Published PyPI baseline is `govengine==0.16.1`.
...
Status: implementation, tests and PyPI publication are complete for `0.16.1`.
...
- keep released consumer dependency floors aligned with the published `0.16.1`
