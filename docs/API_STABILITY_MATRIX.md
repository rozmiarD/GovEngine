...
| --- | --- | --- | --- |
| alpha | govengine.admission | `GovAdmissionDecision`, `GovApprovalRequest`, `GovAuditRecord`, `GovPolicyDecision`, `RuntimeAdmissionResult`, `admission_decision_from_host_gate`, `compose_runtime_admission_result`, `normalize_admission_artifact_refs`, `validate_admission_decision`, `validate_approval_request`, `validate_audit_record`, `validate_policy_decision`, `validate_runtime_admission_result` | Neutral admission/policy/runtime-admission records, bounded reference normalization, and gate-summary composition only; host owns policy meaning, approval workflow, live backend behavior, raw evidence storage, SCLite canonicalization, and production trust/key boundaries. |
...
| alpha | govengine.signing | `SignatureEnvelope`, `SigningPolicy`, `TrustPolicy`, `VerificationResult` | Host-provided signer/verifier/trust decision records; no PKI/KMS/key-store ownership. |
