...
| --- | --- | --- | --- |
| alpha | govengine.admission | `GovAdmissionDecision`, `GovApprovalRequest`, `GovAuditRecord`, `GovPolicyDecision`, `admission_decision_from_host_gate`, `validate_admission_decision`, `validate_approval_request`, `validate_audit_record`, `validate_policy_decision` | Neutral admission/policy records only; host owns policy meaning and approval workflow. |
