...
CURRENT_LIFECYCLE_TRACE_FILES = list(LIFECYCLE_TRACE_FILES_V02)
...
def build_policy_decision_artifact_v02(pipeline_data: Dict[str, Any], intent_contract: Mapping[str, Any]) -> Dict[str, Any]:
    decision = build_policy_decision_artifact(pipeline_data)
    prepared = _prepared(pipeline_data)
    return sanitize_public_artifact({
...
def build_scoped_execution_ticket_v03(
    pipeline_data: Dict[str, Any],
    policy_decision: Mapping[str, Any],
    execution_contract: Mapping[str, Any],
) -> Dict[str, Any]:
    """Build a SCLite scoped ticket for runtime consumption and review."""
    approved = _approved(pipeline_data)
    prepared = _prepared(pipeline_data)
    approval = _dict(approved.get('approval'))
    ...
    ticket = sanitize_public_artifact({
        ...
        'integrity': {
            'ticket_binds_execution_contract_digest': contract_desc['digest'],
...
def build_execution_receipt_v02(pipeline_data: Dict[str, Any], execution_ticket: Mapping[str, Any], execution_contract: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    receipt = sanitize_public_artifact({
        ...
        'runtime': {'name': runtime_name, 'runtime_ref': f'runtime:{runtime_name}', 'mode': mode, 'version': str(pipeline_data.get('runtime_version') or '')},
        'execution': {
    ...
    if execution_ticket.get('schema_version') == 'v0.3':
        receipt['ticket_use'] = {
...
def build_evidence_contract_v02(pipeline_data: Dict[str, Any], execution_receipt: Mapping[str, Any], execution_ticket: Mapping[str, Any]) -> Dict[str, Any]:
    is_scoped = execution_ticket.get('schema_version') == 'v0.3'
    claim = {
    ...
    evidence = sanitize_public_artifact({
...
def build_current_lifecycle_artifacts(pipeline_data: Dict[str, Any]) -> Dict[str, Any]:
    """Build the current SCLite lifecycle using scoped-ticket semantics."""
    intent = build_intent_contract_v02(pipeline_data)
    policy = build_policy_decision_artifact_v02(pipeline_data, intent)
    contract = build_execution_contract_v02(pipeline_data, intent, policy)
